...
//   ├ Correlation pairs & cluster warnings  │ Per-ticker table            ┤
//   └──────────────────────────────────────────────────────────────────────┘
//
// Design intents:
...
import { HBar } from '../_shared/charts'
import { fmtUsd, fmtPct, fmtSignedPct } from '../_shared/format'

const CONCENTRATION_LIMIT = 0.3

export default async function RiskPage() {
  const r = await loadRisk()
  ...
  return (
    <main className="mx-auto max-w-[1520px] space-y-6 p-6">
      ...
      <AlertBanner
        ...
        detail={`of ${fmtUsd(r.totalNetWorthUsd)} total net worth — Thai fund NAVs and cash are excluded from vol/β/VaR (no daily price series).`}
      />
        ...
        <SectionCard title="Correlation structure" className="col-span-12 xl:col-span-5">
          {r.clusters.map((c) => (
            <AlertBanner
              key={c.tickers.join()}
              level="warning"
              title={`Cluster: ${c.tickers.join(' · ')}`}
              detail={`avg pairwise +${Math.round(c.avgCorr * 100)}% — ${c.note}`}
            />
          ))}
          <ul className="mt-3 space-y-1.5">
            {r.topPairs.map((p) => (
              <li key={p.a + p.b} className="flex items-center justify-between text-[13px]">
                <span className="text-ink-2">
                  {p.a} <span className="text-ink-3">↔</span> {p.b}
                  {p.corr < -0.2 && <span className="ml-2 text-[11px] text-ink-3">natural hedge</span>}
                </span>
                {/* diverging encoding: sign carried by label + color, blue↔red */}
                <span className="tnum font-medium" style={{ color: p.corr >= 0 ? 'var(--loss)' : 'var(--accent)' }}>
                  {p.corr > 0 ? '+' : ''}{Math.round(p.corr * 100)}%
                </span>
              </li>
            ))}
          </ul>
          <p className="mt-3 text-[11px] leading-4 text-ink-3">
            Full pairwise heatmap needs correlation.json (backend gap #4) — today only top pairs
            survive the markdown report. Positive correlation reads red (risk stacks), negative blue (hedge).
          </p>
        </SectionCard>
