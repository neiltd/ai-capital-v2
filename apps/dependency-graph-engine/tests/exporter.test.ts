...
describe('exportGraph', () => {
  ...
  beforeEach(async () => {
    ...
    await store.upsertNode({ ticker: 'NVDA', company: 'NVIDIA', themes: ['ai-infrastructure'] })
    await store.upsertNode({ ticker: 'TSM',  company: 'TSMC',   themes: ['semiconductors'] })
  ...
  it('returns correct graph shape', async () => {
    const graph = await exportGraph(store, TEST_OUT)
