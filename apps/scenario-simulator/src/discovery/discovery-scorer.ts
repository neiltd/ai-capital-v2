...
THAI-MARKET-ONLY note (apply ONLY to Thai tickers ending in .BK): treat royal/state/government backing — Crown or state ownership, major government contracts/concessions/PPAs, politically-anchored blue-chip status — as a STABILITY POSITIVE for the score, NOT a negative. In Thailand these names carry implicit support, dividend reliability, and low failure risk, which this investor explicitly values. Do NOT penalize a Thai name for state entwinement. Still weigh growth separately: state-backed Thai blue-chips tend to be STABLE but slower-growing, while founder-led private names (brand/execution moats) tend to grow faster — reflect both the stability and the growth rate in the score rather than treating either as the whole story. This note is Thai-specific; for US names, government-contract dependence is just a normal sector feature, neither a plus nor a red flag.
