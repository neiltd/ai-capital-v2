...
//   ├ Flashing signals table (signal-quality columns)                      ┤
//   ├ Watching / no-signal strip                                           ┤
//   └ Link → /markets/waves/trade (the agent that acts on these)           ┘
//
// Design intents (design-redesign-2026-07/screens/waves):
// - Direction chips: LONG = accent outline, SHORT = warning outline — never
//   gain/green: a signal is a hypothesis, not a profit.
// - This layer is on trial (ROADMAP Phase 4): disagreements with the
//   briefing are surfaced as banners, not hidden — two agents disagreeing
//   is signal. Now backed by a real join against briefings/YYYY-MM-DD.json
//   (gap #1, built earlier this session) instead of the mockup's hardcoded
//   sample conflicts.
...
function DirectionChip({ s }: { s: WaveSignalVM }) {
  const long = s.signal === 'buy'
  return (
    <span className={`inline-flex items-center rounded-chip border px-1.5 py-0.5 text-[11px] font-semibold uppercase tracking-wide ${
...
const money = (v: number | null) => v == null ? '—' : fmtUsd(v, { cents: v < 100 })
    ...
    <main className="mx-auto max-w-[1520px] space-y-sec-gap p-page-pad">
      <header className="flex items-end justify-between">
        <div>
          ...
          <p className="mt-0.5 text-[13px] text-ink-2">
            ...
            no news or fundamentals input. Separate from Discovery by design.
          </p>
        </div>
      ...
      <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
        ...
        <StatTile
          ...
          footnote={`${w.audit.hits} hit / ${w.audit.stopped} stopped of ${w.audit.closed} — hit-rate aggregation is gap #21`}
        />
      ...
      <SectionCard title={`Flashing now · ${w.asOf}`} asOf={w.exportedAt}>
              ...
              {[...w.signals]
                ...
                .map((s) => (
                  <tr key={s.ticker} className="border-b border-hairline last:border-0 hover:bg-surface-2">
                    ...
                    <Td align="right">{money(s.close)}</Td>
                    <Td align="right">
                      {s.entryZone ? `${money(s.entryZone.low)}–${money(s.entryZone.high)}` : '—'}
                    </Td>
                    <Td align="right">{money(s.stopLoss)}</Td>
                    <Td align="right">{money(s.target)}</Td>
        ...
        <p className="mt-2 text-[11px] leading-4 text-ink-3">
          ...
          extension (missing target = unresolved fib cluster). Hover the ticker for the full narrative.
        </p>
      </SectionCard>
