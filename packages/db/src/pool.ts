...
// The canonical explicit-credential validator now lives in its own pg-free
// module so packages/queue can reuse it (S4D). Re-exported here so every
// existing caller — getDashboardPool, agent-claims' writer() and their tests —
// keeps importing it from '@common/db/pool' exactly as before.
import { requireExplicitPostgresUrl } from './credential-url.js'
export { requireExplicitPostgresUrl } from './credential-url.js'
