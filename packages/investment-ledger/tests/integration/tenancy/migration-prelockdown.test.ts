...
// ops/bootstrap/010_database_bootstrap.sql are done, migrations 001-017 have
...
describeInPhase('pre-lockdown', 'the migration chain applied as the migrator', () => {
  ...
  it('all seventeen migrations are recorded, in order, with no gaps', async () => {
    ...
    expect(rows.map(r => r.filename.slice(0, 3)))
      .toEqual(Array.from({ length: 17 }, (_, i) => String(i + 1).padStart(3, '0')))
  })
